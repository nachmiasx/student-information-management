...
import static com.handson.basic.model.StudentSortField.email;
...
public class StudentsController {
    ...
    public ResponseEntity<?> uploadStudentImage(@PathVariable Long id,  @RequestParam("image") MultipartFile image)
    {
        Optional<Student> dbStudent = studentService.findById(id);
        if (dbStudent.isEmpty()) throw new HandsonException("Student with id: " + id + " not found");
        String bucketPath = "apps/aviv/student-" +  id + ".png" ;
    ...
    public ResponseEntity<?> emailAll(@RequestParam String text, @RequestParam String subject)
    {
        new Thread(()-> {
            IteratorUtils.toList(studentService.all().iterator())
                    ...
                    .forEach(email -> {
                        try {
                            emailService.send(text,subject, email);
                        } catch (IOException e) {
                            throw new HandsonException(e.getMessage());
                        }
                    });
        }).start();
        return new ResponseEntity<>("SENDING", HttpStatus.OK);
